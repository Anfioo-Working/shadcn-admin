'use client'

import * as React from 'react'
import { useState, useMemo } from 'react'
import {
  PencilIcon,
  Trash2Icon,
  RefreshCcwIcon,
  PlusIcon,
  DownloadIcon,
  ChevronLeftIcon,
  ChevronRightIcon,
} from 'lucide-react'
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
  AlertDialogDescription,
  AlertDialogFooter,
  AlertDialogHeader,
  AlertDialogTitle,
} from '@/components/ui/alert-dialog'
import { Button } from '@/components/ui/button'
import { Switch } from '@/components/ui/switch'
import { Card, CardContent, CardHeader } from '@/components/ui/card'
import { Checkbox } from '@/components/ui/checkbox'
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/components/ui/select'
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from '@/components/ui/table'
import {
  configList,
  deleteConfig,
  refreshConfigCache,
  updateConfig,
} from '@/features/system/shared/api'
import type { Config, ConfigForm } from '@/features/system/shared/types'
import type { ConfigSearchParams } from './config-search-form'

interface ConfigDataTableProps {
  searchParams: ConfigSearchParams
  onEdit: (config: Config) => void
  onAdd: () => void
}

export function ConfigDataTable({
  searchParams,
  onEdit,
  onAdd,
}: ConfigDataTableProps) {
  const [configs, setConfigs] = useState<Config[]>([])
  const [total, setTotal] = useState(0)
  const [pageNum, setPageNum] = useState(1)
  const [pageSize, setPageSize] = useState(10)
  const [loading, setLoading] = useState(false)
  const [selectedIds, setSelectedIds] = useState<number[]>([])
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<number[] | null>(null)
  const [refreshCacheDialogOpen, setRefreshCacheDialogOpen] = useState(false)

  // 加载配置列表
  const loadConfigs = React.useCallback(async () => {
    setLoading(true)
    try {
      const res = await configList({
        pageNum,
        pageSize,
        configName: searchParams.configName,
        configKey: searchParams.configKey,
        configType: searchParams.configType,
      })
      if (res.code === 200) {
        setConfigs(res.data.rows)
        setTotal(res.data.total)
      }
    } finally {
      setLoading(false)
    }
  }, [pageNum, pageSize, searchParams])

  React.useEffect(() => {
    void (async () => {
      loadConfigs()
    })()
  }, [loadConfigs])

  // 选择相关
  const isAllSelected = useMemo(() => {
    return configs.length > 0 && selectedIds.length === configs.length
  }, [configs, selectedIds])

  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(configs.map((c) => c.configId))
    } else {
      setSelectedIds([])
    }
  }

  const handleSelectRow = (configId: number, checked: boolean) => {
    if (checked) {
      setSelectedIds([...selectedIds, configId])
    } else {
      setSelectedIds(selectedIds.filter((id) => id !== configId))
    }
  }

  // 删除操作
  const handleDeleteClick = (ids: number[]) => {
    setDeleteTarget(ids)
    setDeleteDialogOpen(true)
  }

  const handleDeleteConfirm = async () => {
    if (!deleteTarget) return
    for (const id of deleteTarget) {
      await deleteConfig(id)
    }
    setDeleteDialogOpen(false)
    setDeleteTarget(null)
    setSelectedIds([])
    loadConfigs()
  }

  // 刷新缓存操作
  const handleRefreshCacheClick = () => {
    setRefreshCacheDialogOpen(true)
  }

  const handleRefreshCacheConfirm = async () => {
    const res = await refreshConfigCache()
    if (res.code === 200) {
      setRefreshCacheDialogOpen(false)
      loadConfigs()
    }
  }

  // 分页
  const totalPages = Math.ceil(total / pageSize)

  const handlePrevPage = () => {
    if (pageNum > 1) {
      setPageNum(pageNum - 1)
    }
  }

  const handleNextPage = () => {
    if (pageNum < totalPages) {
      setPageNum(pageNum + 1)
    }
  }

  const handlePageSizeChange = (newSize: number) => {
    setPageSize(newSize)
    setPageNum(1)
  }

  // 工具栏按钮禁用状态
  const canEdit = selectedIds.length === 1
  const canDelete = selectedIds.length > 0

  return (
    <Card>
      <CardHeader className='border-b px-4 py-3'>
        <div className='flex items-center gap-2'>
          <Button onClick={onAdd}>
            <PlusIcon data-icon='inline-start' />
            新增
          </Button>
          <Button
            className='bg-green-600 hover:bg-green-700 text-white'
            disabled={!canEdit}
            onClick={() => {
              const config = configs.find((c) => c.configId === selectedIds[0])
              if (config) onEdit(config)
            }}
          >
            <PencilIcon data-icon='inline-start' />
            修改
          </Button>
          <Button
            variant='destructive'
            disabled={!canDelete}
            onClick={() => handleDeleteClick(selectedIds)}
          >
            <Trash2Icon data-icon='inline-start' />
            删除
          </Button>
          <Button className='bg-amber-500 hover:bg-amber-600 text-white'>
            <DownloadIcon data-icon='inline-start' />
            导出
          </Button>
          <Button variant='outline' onClick={handleRefreshCacheClick}>
            <RefreshCcwIcon data-icon='inline-start' />
            刷新缓存
          </Button>
        </div>
      </CardHeader>
      <CardContent className='p-0'>
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className='w-[50px]'>
                <Checkbox
                  checked={isAllSelected}
                  onCheckedChange={handleSelectAll}
                  aria-label='全选'
                />
              </TableHead>
              <TableHead>参数名称</TableHead>
              <TableHead>参数键名</TableHead>
              <TableHead>参数键值</TableHead>
              <TableHead>系统内置</TableHead>
              <TableHead>备注</TableHead>
              <TableHead>创建时间</TableHead>
              <TableHead className='w-[150px]'>操作</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {loading ? (
              <TableRow>
                <TableCell
                  colSpan={8}
                  className='py-8 text-center text-muted-foreground'
                >
                  加载中...
                </TableCell>
              </TableRow>
            ) : configs.length === 0 ? (
              <TableRow>
                <TableCell
                  colSpan={8}
                  className='py-8 text-center text-muted-foreground'
                >
                  暂无数据
                </TableCell>
              </TableRow>
            ) : (
              configs.map((config) => (
                <TableRow
                  key={config.configId}
                  data-selected={selectedIds.includes(config.configId)}
                >
                  <TableCell>
                    <Checkbox
                      checked={selectedIds.includes(config.configId)}
                      onCheckedChange={(checked) =>
                        handleSelectRow(config.configId, checked as boolean)
                      }
                      aria-label='选择行'
                    />
                  </TableCell>
                  <TableCell>{config.configName}</TableCell>
                  <TableCell>{config.configKey}</TableCell>
                  <TableCell>{config.configValue}</TableCell>
                  <TableCell>
                    <Switch
                      checked={config.configType === 'Y'}
                      onCheckedChange={async (checked) => {
                        const newConfigType = checked ? 'Y' : 'N'
                        await updateConfig({
                          ...config,
                          configType: newConfigType,
                        } as ConfigForm)
                        loadConfigs()
                      }}
                    />
                  </TableCell>
                  <TableCell>{config.remark || '-'}</TableCell>
                  <TableCell>{config.createTime}</TableCell>
                  <TableCell>
                    <div className='flex items-center gap-1'>
                      <Button
                        variant='ghost'
                        size='sm'
                        onClick={() => onEdit(config)}
                        className='h-7 px-2'
                      >
                        <PencilIcon className='size-3.5' />
                        修改
                      </Button>
                      <Button
                        variant='ghost'
                        size='sm'
                        onClick={() => handleDeleteClick([config.configId])}
                        className='h-7 px-2'
                      >
                        <Trash2Icon className='size-3.5' />
                        删除
                      </Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
        </Table>

        {/* 分页 */}
        {total > 0 && (
          <div className='flex items-center justify-between border-t px-4 py-3'>
            <div className='flex items-center gap-2 text-sm text-muted-foreground'>
              共 {total} 条
              <Select
                value={String(pageSize)}
                onValueChange={(v) => handlePageSizeChange(Number(v))}
              >
                <SelectTrigger className='w-[80px]'>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value='10'>10条/页</SelectItem>
                  <SelectItem value='20'>20条/页</SelectItem>
                  <SelectItem value='50'>50条/页</SelectItem>
                  <SelectItem value='100'>100条/页</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className='flex items-center gap-2'>
              <Button
                variant='outline'
                size='sm'
                disabled={pageNum === 1}
                onClick={handlePrevPage}
              >
                <ChevronLeftIcon className='size-4' />
              </Button>
              <span className='text-sm'>
                第 {pageNum} / {totalPages} 页
              </span>
              <Button
                variant='outline'
                size='sm'
                disabled={pageNum === totalPages}
                onClick={handleNextPage}
              >
                <ChevronRightIcon className='size-4' />
              </Button>
            </div>
          </div>
        )}
      </CardContent>

      {/* 删除确认对话框 */}
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>确认删除</AlertDialogTitle>
            <AlertDialogDescription>
              是否确认删除参数编号为 "{deleteTarget?.join(', ')}" 的数据项？
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>取消</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteConfirm}>
              确定
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>

      {/* 刷新缓存确认对话框 */}
      <AlertDialog
        open={refreshCacheDialogOpen}
        onOpenChange={setRefreshCacheDialogOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>确认刷新缓存</AlertDialogTitle>
            <AlertDialogDescription>
              是否确认刷新参数缓存？
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>取消</AlertDialogCancel>
            <AlertDialogAction onClick={handleRefreshCacheConfirm}>
              确定
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </Card>
  )
}
